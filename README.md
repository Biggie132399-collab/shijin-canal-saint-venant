...
1. 若要严格使用老师现有一维软件，应明确老师模型的输入输出格式、边界条件格式和数值离散格式。
2. 当前 HLL 通量仅为自编原型的离散选择，不应表述为“咱们一维软件的离散格式”。
3. 若拿到老师模型的 `exe`，可将本仓库整理为前处理、批量运行和后处理框架。
4. 若拿到老师模型的 `source code`，可进一步把当前文档中的数值离散公式改成与源代码完全一致。
